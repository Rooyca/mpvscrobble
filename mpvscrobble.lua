...
local function load_cached_session()
    ...
    if file then
        ...
        if session_key and session_key ~= "" then
            session_key = session_key:gsub("^%s*(.-)%s*$", "%1") -- trim whitespace
...
function LastFM:request(endpoint, params)
    ...
    if data.error then
        -- If session expired (error 9), clear cache and try again
...
local function parse_icy_title(icy_title)
    ...
    msg.debug("Parsing: " .. icy_title)

    icy_title = icy_title:gsub(" on [^%-]+$", "")
    icy_title = icy_title:gsub(" %- [^%-]+$", "")

    local title, artist = icy_title:match("^(.+)%s+by%s+(.+)$")
...
local function on_metadata_change()
    ...
    local artist = metadata.artist or metadata.ARTIST or
                   metadata.icy_artist or metadata["icy-artist"]
    local title = metadata.title or metadata.TITLE or
                  metadata.icy_title or metadata["icy-title"] or
                  metadata["ICY-TITLE"]
    ...
    if artist and title then
        ...
        msg.debug("Using metadata tags - Artist: " .. parsed.artist .. ", Title: " .. parsed.title)
    elseif title and options.parse_patterns then
        parsed = parse_icy_title(title)
        if parsed then
            msg.debug("Parsed from icy-title - Artist: " .. parsed.artist .. ", Title: " .. parsed.title)
        end
    end

    if not parsed and title then
        parsed = {artist = "Unknown Artist", title = title}
        msg.debug("Using raw title: " .. title)
    end

    if not parsed then
        msg.debug("No metadata found in stream")
        return
    end

    if not current_track or
       current_track.title ~= parsed.title or
       current_track.artist ~= parsed.artist then
...
local function on_track_change()
    ...
    if metadata then
        if not current_track or
           current_track.title ~= metadata.title or
           current_track.artist ~= metadata.artist then
